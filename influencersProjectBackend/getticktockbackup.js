...
const USER_UNIQUE_ID = "tiktok marketing influencers";
...
// This the final URL you make 
// a request to for the API call, it is ALWAYS this, do not mistaken it for the signed URL
...
async function main() {
  ...
  try {
    ...
    console.log("Raw response data:", res.data); // Log to verify the response structure
    const channelData = processTikTokData(res.data);
    console.log("Processed channel data:", channelData);  // Log the final structured data
  } catch (error) {
    console.error("Error fetching TikTok data:", error);  // Handle any API errors
  }
}

// Function to process and extract relevant TikTok data
function processTikTokData(data) {
  // Log the entire response to see the structure
  console.log("Processing TikTok data:", data);

  // Check if the user data exists and contains the necessary fields
  if (!data || !data.user || !data.user.stats) {
    console.error("Invalid TikTok data format or missing fields.");
    return null;
  }

  const user = data.user;
  const stats = user.stats;
  const description = user.signature || "No description available";

  // Structure the extracted channel data
  const channelData = {
    name: user.nickname || "Unknown",
    channelName: user.unique_id || "Unknown",
    followersCount: stats.follower_count || 0,
    totalViews: stats.video_play_count || 0,
    videoCount: stats.video_count || 0,
    channelDescription: description,
    contactInfo: user.email || "Not available",
    relevantVideoLinks: [],  // Add links to videos if you can fetch them
  };
